...
def test_bin_get_distance():
    ...
    l1 = inversion.Line([0,0,-1], [0,0,1])
    l2 = inversion.Line([-1,1,0], [1,1,0])
    test_bin = inversion.Bin(l1)
    dist = test_bin.get_distance_to_line(l2)
    goal_dist = 1
    assert dist == goal_dist, "Distance of " + str(l1) + " and " + str(l2) + " should be " + str(goal_dist) + " but was " + str(dist)

    # intersecting cross
    l1 = inversion.Line([0,0,-1], [0,0,1])
    l2 = inversion.Line([-1,0,0], [1,0,0])
    test_bin = inversion.Bin(l1)
    dist = test_bin.get_distance_to_line(l2)
    goal_dist = 0
    assert dist == goal_dist, "Distance of " + str(l1) + " and " + str(l2) + " should be " + str(goal_dist) + " but was " + str(dist)
    ...
    l1 = inversion.Line([0,0,-1], [0,0,1])
    l2 = inversion.Line([0,1,-1], [0,1,1])
    test_bin = inversion.Bin(l1)
    dist = test_bin.get_distance_to_line(l2)
    goal_dist = 1
    assert dist == goal_dist, "Distance of " + str(l1) + " and " + str(l2) + " should be " + str(goal_dist) + " but was " + str(dist)

    # parallel lines which do not touch but are "aligned", i.e. are on the same infinite line
    l1 = inversion.Line([0,0,-2],[0,0,-1])
    l2 = inversion.Line([0,0,1],[0,0,2])
    test_bin = inversion.Bin(l1)
    dist = test_bin.get_distance_to_line(l2)
    goal_dist = 2
    assert dist == goal_dist, "Distance of " + str(l1) + " and " + str(l2) + " should be " + str(goal_dist) + " but was " + str(dist)
    # TODO: more tests, cover all possible cases of relations of finite lines in 3D.
