...
def generate_measurements(receiver_position, sender_positions, room):
    ...
    # Flatten list
    room_triangle_indices = [item for sublist in room_triangle_indices for item in sublist]
    # Compute triangles
    room_triangles = [[room.vertices[face[0]], room.vertices[face[1]], room.vertices[face[2]]] for face in room_triangle_indices]
    for sender_position in sender_positions:
        # Compute direct signal direction and length
        direct_signal_direction = np.subtract(sender_position, receiver_position)
        direct_signal_length = np.linalg.norm(direct_signal_direction)
        direct_signal_direction = np.divide(direct_signal_direction, np.linalg.norm(direct_signal_direction))
        direct_signal = ildars.DirectSignal(direct_signal_direction)
        # TODO: Check if the direct signal can actually be receiver or is obstructed by a wall
        ...
        for triangle in room_triangles:
            ...
            if intersection_point is None:
                ...
                continue
            mirror_point = np.add(receiver_position, np.multiply(np.subtract(intersection_point, receiver_position), 2))
            # Compute and normalize reflection direction and intersect with wall
            reflection_direction = np.subtract(mirror_point, sender_position)
            reflection_direction = np.divide(reflection_direction, np.linalg.norm(reflection_direction))
            reflection_point = get_line_triangle_intersection(sender_position, reflection_direction, triangle)
    ...
    # for debugging: print numbers of direct and reflected signals found
    print("found", len(direct_signals), "direct signals")
    print("found", len(reflected_signals), "reflected signals")
    return (direct_signal, reflected_signals)
